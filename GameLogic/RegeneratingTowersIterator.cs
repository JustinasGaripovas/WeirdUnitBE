...
{
    public class RegeneratingTowersIterator : IIterator
    {
        ...
        public void First()
        {           
            this._position = this._collection.GetCollection().TakeWhile(tower => !(tower is RegeneratingTower)).Count();
        }
        public bool IsDone()
        {
        ...
        {
           int index = this._collection.GetCollection().Skip(this._position + 1).TakeWhile(tower => !(tower is RegeneratingTower)).Count();
           this._position += index + 1;         
        }

        //for(First, isdone, movenext)
    }
}